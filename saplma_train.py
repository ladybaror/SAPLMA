...
# ------------------------------------------------------------
# Modes:
#   1) Leave-one-dataset-out (default): trains on all-but-one dataset,
#      evaluates on the held-out, repeats for each dataset, builds a table.
#   2) Train-on-all (config.TRAIN_ON_ALL=True):
#      - Concatenate all datasets to train a single model.
#      - Pick threshold on a dev split (no leakage).
#      - Optionally evaluate once on a disjoint final holdout set.
#      - Save a bundle under pretrained_saplma/.../ALL_layer<k>/
# ------------------------------------------------------------
...
def run_train_on_all(layer_num_from_end: int, datasets: List[pd.DataFrame]) -> None:
    """
    Concatenate all datasets, optionally carve a final holdout,
    pick threshold on dev, optionally report final holdout acc@thr,
    and save bundle under ALL_layer<k>/.
    """
    print(f"\n[ALL] Mode active for layer {layer_num_from_end}.")
    all_df = pd.concat(datasets, ignore_index=True)

    if all_df.empty:
        raise ValueError("[ALL] Combined dataframe is empty.")

    all_emb = parse_embeddings_column(all_df["embeddings"])
    all_y   = all_df["label"].to_numpy()
    input_dim = all_emb.shape[1]

    # Optional final holdout (never used for threshold selection)
    if EVAL_ON_HOLDOUT:
        X_rest, X_hold, y_rest, y_hold = train_test_split(
            all_emb, all_y,
            test_size=FINAL_HOLDOUT_FRACTION,
            stratify=all_y,
            random_state=SEED
        )
        print(f"[ALL] Final holdout size: {len(y_hold)} ({FINAL_HOLDOUT_FRACTION*100:.1f}%)")
    else:
        X_rest, y_rest = all_emb, all_y
        X_hold = y_hold = None

    # Train/dev split (dev for early stopping and threshold selection)
    ...
    y_dev_prob = model.predict(X_dev, verbose=0).ravel()
    optimal_threshold = choose_optimal_threshold(y_dev, y_dev_prob)

    metric_name = "dev_acc@optimal_threshold"
    metric_val = None

    if EVAL_ON_HOLDOUT:
        y_hold_prob = model.predict(X_hold, verbose=0).ravel()
        y_hold_pred = (y_hold_prob > optimal_threshold).astype(int)
        hold_acc = accuracy_score(y_hold, y_hold_pred)
        print(f"[ALL] Final holdout acc@thr: {hold_acc:.4f}")
        metric_name = "holdout_acc@optimal_threshold"
        metric_val = float(hold_acc)

    bundle_dir = OUTPUT_DIR / f"ALL_layer{abs(layer_num_from_end)}"
    meta = {
        "mode": "train_on_all",
        "model_name": MODEL_NAME,
        "layer_from_end": int(layer_num_from_end),
        "heldout_dataset": "GLOBAL_HOLDOUT" if EVAL_ON_HOLDOUT else None,
        "train_datasets": LIST_OF_DATASETS,
    ...
    print(f"[ALL] Saved ALL-data SAPLMA bundle to: {bundle_dir}")
    print(f"[ALL] Optimal threshold (dev): {optimal_threshold:.4f}")
...
def main():
    ...
    # Track best classifier across ALL layers/datasets/repeats (LOO mode only)
    best_global = {
        "acc": -1.0,
        "dir": None,
        "meta": None,
    ...
    for layer_num_from_end in LAYERS_TO_USE:
        ...
        # ---------- Train-on-all path ----------
        if TRAIN_ON_ALL:
            run_train_on_all(layer_num_from_end, datasets)
            # Skip LOO table entirely in this mode; continue to next layer
            continue

        # ---------- Default: Leave-one-dataset-out ----------
        ...
        for ds in range(dataset_loop_length):
            ...
            for i in range(REPEAT_EACH):
                ...
                model = build_model(train_embeddings.shape[1])
    ...
    # If we used train-on-all for all layers, there's no LOO table to print/save
    if TRAIN_ON_ALL:
        print("\n[ALL] Train-on-all mode finished for all layers.")
        return

    # ---------- Final table & overall averages (LOO only) ----------
