...
# - **NEW**: number+'.' is *not* EOS unless that *same* numeric token already contains a dot;
#           i.e., '3.' ≠ EOS, but '3.14.' (second dot) = EOS.
# ---------------------------------------------------------------------------
...
class SentenceTokenizer:
    ...
    # --- NEW numeric rules helpers ---

    def _is_second_dot_in_number(self, text: str, dot_idx: int) -> bool:
        """
        True iff the '.' at dot_idx is immediately after a digit and there is already
        another '.' earlier within the SAME numeric token (scan left across digits/dots).
        """
        j = dot_idx - 1
        # allow spaces between digit and dot (rare, but mirror prior logic)
        while j >= 0 and text[j].isspace():
            j -= 1
        if j < 0 or not text[j].isdigit():
            return False
        # Scan left within [digits|dots]
        k = j
        seen_prev_dot = False
    ...
    def _ends_with_single_number_dot(self, s: str) -> bool:
        """
        Returns True if string ends with a *single* number+'.' (optionally followed by quotes/brackets),
        meaning that numeric token had NO prior '.' inside it (e.g., '3.').
        In that case, treat as NOT a true sentence end.
        """
        if not s:
            return False
        t = s.rstrip()
        # Strip trailing quotes/brackets that may be after punctuation
        while t and t[-1] in "\"')]}":
            t = t[:-1].rstrip()
        if not t or t[-1] != ".":
            return False
        # char before '.' must be a digit
        if len(t) < 2 or not t[-2].isdigit():
            return False
        # Walk left inside the numeric token
        k = len(t) - 2
        seen_prev_dot = False
        while k >= 0 and (t[k].isdigit() or t[k] == "."):
            if t[k] == ".":
                seen_prev_dot = True
            k -= 1
        return not seen_prev_dot  # True => it's a single number+'.'
    ...
    def _regex_first_complete(self, text: str) -> Optional[str]:
        ...
        for i, ch in enumerate(text):
            ...
            if ch == "." and self._is_abbrev_before(text, i):
                continue
            # Numeric rule: if '.' follows a digit, it's only EOS if it's the second '.' within that number
            if ch == ".":
                j = i - 1
                while j >= 0 and text[j].isspace():
                    j -= 1
                if j >= 0 and text[j].isdigit():
                    if not self._is_second_dot_in_number(text, i):
                        # First '.' after a plain integer -> not EOS
                        continue
            ...
            # boundary if end of string OR next is whitespace
            if end >= n or text[end].isspace():
                cand = text[:end].strip()
                fixed = self._postfix_enumeration_rules(cand)
                if fixed:
                    # Don't finalize on a trailing single number+'.'
...
def _bad_first_piece(piece: str) -> bool:
    ...
    if len(s) == 1 and s.isalpha() and s.upper() not in ("I", "A"):
        return True
    # odd non-ASCII leading letter (e.g., 'Љ')c
...
# # - **NEW**: strip trailing whitespace tokens (e.g., SentencePiece '▁') from accepted_ids.
# # - **NEW**: optional `saplma_threshold` parameter to override the bundle’s threshold.
# # - **NEW**: do NOT treat a '.' preceded by a digit as an end-of-sentence across all backends.
# # ---------------------------------------------------------------------------
...
# # ---------------------------------------------------------------------------
# # Sentence tokenizer (spaCy -> NLTK -> regex) WITH digit-before-dot rule
# # ---------------------------------------------------------------------------
# class SentenceTokenizer:
#     _ABBREV = {
#         "mr","mrs","ms","dr","prof","sr","jr","st","vs","etc",
...
#         # colon+newline can terminate a sentence
#         m_colon = re.search(r":\s*(?:\r?\n)", text)
#         if m_colon:
#             cand = text[:m_colon.start()+1].strip()
#             fixed = self._postfix_enumeration_rules(cand)
#             # Also reject if ends with digit-period
