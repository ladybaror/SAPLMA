...
from saplma_guarded_generate_completion_auto import (
...
def load_prompts(prompts_arg: Optional[str]) -> List[str]:
    if prompts_arg is None:
        return [
            "Dogs are loyal and also",
            "The capital of France is",
            "Explain what overfitting means in one sentence",
        ]
    p = Path(prompts_arg)
    if not p.exists():
...
"""
python completion_model/run_batch_compare.py \
  ...
  --prompts completion_model/prompts.txt \
  --out completion_model/results.jsonl
