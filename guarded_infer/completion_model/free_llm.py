...
# - Builds and PASSES an attention_mask to generate()
...
import torch
from transformers import AutoTokenizer, AutoModelForCausalLM, PreTrainedTokenizerBase as AutoTokenizerBase
...
try:
    ...
    def _render_for_generation(tok: AutoTokenizerBase, messages: List[dict]) -> List[int]:
        try:
    ...
    def _strip_trailing_space_tokens(ids: List[int], tok: AutoTokenizerBase) -> List[int]:
        j = len(ids)
        while j > 0:
...
# --- Small utils ---
def _ensure_pad(tok: AutoTokenizerBase):
...
def _newline_token_ids(tok: AutoTokenizerBase) -> Set[int]:
    s: Set[int] = set()
    for seq in ("\n", "\r\n", "\n\n"):
        try:
...
def _render_for_generation_inputs(tok: AutoTokenizerBase, messages: List[dict]):
    """
    Return (input_ids_tensor, attention_mask_tensor) using chat template -> tokenizer.
    This is robust and will remain correct if you later add batching/padding.
    """
    try:
        ...
        # Fallback: raw prompt + a space to avoid leading-join issues
        prompt = messages[0]["content"]
        rendered = prompt if prompt.endswith((" ", "\n")) else prompt + " "

    enc = tok(
        rendered,
        return_tensors="pt",
        add_special_tokens=False,
        padding=False,      # no batch padding here
        truncation=False,   # let generate handle length/kv-cache
    )
    # Shapes: [1, seq_len]
...
def generate_without_guardrail(
    ...
    **unused,
    ...
    # Robust: render → tokenize → get ids + mask
    input_ids, attention_mask = _render_for_generation_inputs(tok, messages)
    input_ids = input_ids.to(mdl.device)
    attention_mask = attention_mask.to(mdl.device)

    # Keep list form to later slice the generated tail correctly
    # (We still compute the "template" ids via helper for parity with your trimming logic.)
