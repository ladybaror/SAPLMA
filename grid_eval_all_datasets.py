...
def draw_ascii_table(dataset: str, cell_text: Dict[Tuple[int,int], str]) -> None:
    col_keys = TRAIN_FORMATS
    row_keys = TEST_FORMATS
    header0 = f"test({dataset}) \\ train({dataset})"
...
def write_markdown_table(md_path: Path, dataset: str, cell_text: Dict[Tuple[int,int], str]) -> None:
    col_keys = TRAIN_FORMATS
    row_keys = TEST_FORMATS
    headers = [f"test({dataset}) \\ train({dataset})"] + [FMT_LABEL[c] for c in col_keys]
...
def write_matrix_csv(csv_path: Path, cell_vals: Dict[Tuple[int,int], float]) -> None:
    # rows = test_fmt; columns = train_fmt; values = acc@0.5 (0..1)
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["test\\train"] + [FMT_LABEL[c] for c in TRAIN_FORMATS])
        for r in TEST_FORMATS:
            w.writerow([FMT_LABEL[r]] + [f"{cell_vals[(r,c)]:.6f}" for c in TRAIN_FORMATS])
...
def main():
    ...
    # Collect a long-form summary for all datasets
    ...
    for dataset in DATASET_NAMES:
        ...
        draw_ascii_table(dataset, cell_text)

        # Save markdown + csv matrix
        ds_out_dir = OUT_ROOT / dataset
        ds_out_dir.mkdir(parents=True, exist_ok=True)
        write_markdown_table(ds_out_dir / "matrix.md", dataset, cell_text)
        write_matrix_csv(ds_out_dir / "matrix.csv", cell_vals)
